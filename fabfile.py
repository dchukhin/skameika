...
def setup_master():
    ...
    with settings(host_string=env.master):
        ...
        files.upload_template(
            filename="conf/master.tmpl",
            destination="/etc/salt/master",
            context={
                "loglevel": DEFAULT_SALT_LOGLEVEL,
...
def setup_minion(*roles):
    ...
    if not env.host_string:
        abort(
            'When calling "setup_minion", you must pass "-H <hostname|ipaddress> " '
...
def add_role(name):
    ...
    if not env.host_string:
        abort(
            'When calling "add_role", you must pass "-H <hostname|ipaddress> " '
...
def generate_gpg_key():
    ...
    with settings(host_string=env.master):
        if not files.exists(os.path.join(gpg_home, "secring.gpg"), use_sudo=True):
            ...
            files.upload_template(
                filename="conf/keys/gpg.tmpl",
                destination=gpg_file,
    ...
    with hide("running", "stdout", "stderr"):
        ...
        for name in args:
            local(
                ...
                "--trust-model always -aer {}_salt_key {}".format(temp_key, env.environment, name)
            )
        # Encrypt each value
        updates = {}
            ...
            updates[name] = "{}".format(
                local(
                    ...
                    "--trust-model always -aer {}_salt_key".format(
                        value, temp_key, env.environment
                    ),
                    capture=True,
                )
            )
...
def hostnames_for_role(role):
    with hide("running", "stdout"):
        result = salt(cmd="grains.item fqdn --output=yaml", target='-G "roles:%s"' % role)
...
def manage_run(command):
    ...
    manage_sh = "/var/www/%s/manage.sh " % project_name
