...
lint-py:
	...
	# Requires flake8
	$(WORKON_HOME)/skameika/bin/flake8 .
...
setup:
	virtualenv -p `which python3.5` $(WORKON_HOME)/skameika
	$(WORKON_HOME)/skameika/bin/pip install -U pip wheel
	$(WORKON_HOME)/skameika/bin/pip install -Ur requirements/dev.txt
	$(WORKON_HOME)/skameika/bin/pip freeze
	npm install
	npm update
	cp skameika/settings/local.example.py skameika/settings/local.py
	echo "DJANGO_SETTINGS_MODULE=skameika.settings.local" > .env
	createdb -E UTF-8 skameika
	$(WORKON_HOME)/skameika/bin/python manage.py migrate
	if [ -e project.travis.yml ] ; then mv project.travis.yml .travis.yml; fi
	@echo
	@echo "The skameika project is now setup on your machine."
	@echo "Run the following commands to activate the virtual environment and run the"
	@echo "development server:"
	@echo
	@echo "	workon skameika"
	@echo "	npm run dev"

update:
	$(WORKON_HOME)/skameika/bin/pip install -U -r requirements/dev.txt
	npm install
	npm update
